...
import { Discount, UpdateDiscountRequest } from '../../types/discounts';

interface EditDiscountModalProps {
  discount: Discount;
  onClose: () => void;
  onUpdate: (id: string, data: UpdateDiscountRequest) => Promise<boolean>;
}

const EditDiscountModal: React.FC<EditDiscountModalProps> = React.memo(({ discount, onClose, onUpdate }) => {
  const [formData, setFormData] = useState<UpdateDiscountRequest>({
    ...
    maximum_discount_amount: discount.maximum_discount_amount || 0,
  ...
  const handleInputChange = (field: keyof UpdateDiscountRequest, value: string | number | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.minimum_order_amount && formData.minimum_order_amount < 0) {
    ...
    if (formData.usage_limit && formData.usage_limit < discount.used_count) {
      setError(`Usage limit cannot be less than current usage (${discount.used_count})`);
      return;
    }

    try {
      setIsSubmitting(true);
      const success = await onUpdate(discount.id, formData);
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Code: <span className="font-mono font-medium">{discount.code}</span></p>
          </div>
          <button
            onClick={onClose}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      value={formData.maximum_discount_amount || 0}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Current usage: {discount.used_count} (0 = unlimited)
                </p>
              </div>
